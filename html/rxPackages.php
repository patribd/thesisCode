...
$header = "";
$i = 0;
$printCode = file("skin/rxPackages.html");
...
$code = file_get_contents("skin/rxPackages.html");
...
$devices = $db -> prepare ("SELECT * FROM rxPackages ORDER BY floatTimestamp DESC LIMIT 10 OFFSET " . $offset);
...
while ($row = $devices -> fetch()){
    ...
    $transmissionTime = $db -> prepare("SELECT * FROM txPackages WHERE packageID=:packageID");
    unset($contentData);
    $contentData['packageID']=$row['packageID'];
    $p1 = $transmissionTime -> execute($contentData);
    $p2 = $transmissionTime -> fetch();
    if ($p2!=null){
        $time = $row['floatTimestamp'] - $p2['floatTimestamp'];
    }else{
        $content_device = str_replace("##transmissionTime##", "The package with the transmission information was not received yet.", $content_device);
    }
    $content_device = str_replace("##transmissionTime##", $time, $content_device);
...
$total=$db -> prepare ("SELECT COUNT(*) FROM rxPackages");
$p1=$total->execute();
